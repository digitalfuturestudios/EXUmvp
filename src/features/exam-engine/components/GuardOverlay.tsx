// ============================================================
// GuardOverlay — Full-screen overlay rendered on guard violations.
// Presents a countdown and a fullscreen request prompt.
// ============================================================
...
function BlockedView({
  ...
  return (
    <motion.div
      className="flex max-w-md flex-col items-center gap-6 rounded-2xl border border-amber-500/30 bg-amber-950/80 p-10 text-center shadow-2xl"
    ...
    >
      ...
      {!isFullscreen && (
        <button
          ...
          className="mt-2 flex items-center gap-2 rounded-lg bg-amber-500 px-5 py-2.5 text-sm font-semibold text-black transition-opacity hover:opacity-90"
        >
...
function SuspendedView({ strikes, maxStrikes }: { strikes: number; maxStrikes: number }): JSX.Element {
  ...
  return (
    <motion.div
      className="flex max-w-md flex-col items-center gap-6 rounded-2xl border border-red-500/30 bg-red-950/80 p-10 text-center shadow-2xl"
      initial={{ scale: 0.8, y: 20 }}
      animate={{ scale: 1, y: 0 }}
      transition={{ type: 'spring', stiffness: 200 }}
    >
